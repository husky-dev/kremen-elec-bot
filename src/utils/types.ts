...
export const keys = <D extends object>(value: D): string[] => Object.keys(value);
...
export const splitByChunks = <T>(items: T[], size: number): T[][] => {
  if (!items.length) return [];
  const chunks: T[][] = [[]];
  for (let i = 0; i < items.length; i++) {
    if (chunks[chunks.length - 1].length >= size) chunks.push([]);
    chunks[chunks.length - 1].push(items[i]);
  }
  return chunks;
};
